...
S30 Training per-trial plot
```{r}
sr_30_exp_step1_time <- sr_30_ppt_training %>%
  group_by(trial_num) %>% # note: only one where don't need to group_by ppid
  summarise(
...
p <- data %>%
  ggplot(
    aes(
      x = trial_num - 138, y = mean_step1_time
    )
  ) +
  theme_classic() +
  theme(legend.position = "none") +
...
group1 <- data_ppt %>% 
  filter(trial_set == "1") %>% 
  pull(mean_trial_set_step1_time)
...
```{r}
sr_30_exp_step2_rt <- sr_30_ppt_training %>%
  group_by(trial_num) %>% # note: only one where don't need to group_by ppid
  summarise(
...
p <- data %>%
  ggplot(
    aes(
      x = trial_num - 138, y = mean_step2_rt
    )
  ) +
  theme_classic() +
  theme(legend.position = "none") +
