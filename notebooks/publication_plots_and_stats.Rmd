...
```{r}
temp_ANOVA <- aov(ppt_mean_angle ~ obj_shape_sign, data = dr_30_ppt_incl_clamped)
...
```
```{r}
bf <- anovaBF(ppt_mean_angle ~ obj_shape_sign, data = data.frame(dr_30_ppt_incl_clamped), progress = FALSE)
print(bf)
bayesfactor_inclusion(bf)
```
...
```{r}
temp_ANOVA <- aov(ppt_mean_angle ~ obj_shape_sign, data = dr_30_ppt_excl_clamped)
...
```
```{r}
bf <- anovaBF(ppt_mean_angle ~ obj_shape_sign, data = data.frame(dr_30_ppt_excl_clamped), progress = FALSE)
print(bf)
bayesfactor_inclusion(bf)
```
...
```{r}
# both_ppt_training <- both_ppt_training_pre %>%
#   group_by(ppid, exp) %>%
#   summarise(
